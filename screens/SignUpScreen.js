  ...
  const handleGoogleSignIn = async () => {
    if (!googlePromptAsync) {
      Alert.alert(
        ...
        t('auth.googleNotConfigured', { defaultValue: 'Google Sign-In is not configured. Please try email/password registration.' })
      );
      return;
    }
    ...
    try {
      ...
      if (!result.success) {
        ...
        if (result.error === 'cancelled') {
          console.log('[SignUp] Google sign-in cancelled by user');
        ...
        Alert.alert(
          t('auth.signUpFailed', { defaultValue: 'Sign Up Failed' }),
      ...
      console.error('[SignUp] Google sign-in error:', error.message);
      Alert.alert(
        t('auth.signUpFailed', { defaultValue: 'Sign Up Failed' }),
